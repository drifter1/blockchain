...
class UTXO:
    def __init__(self, utxo_outputs: list = None, balance: float = None, last_block_included: int = None):
...
def json_construct_utxo(utxo: UTXO):
    return{
        ...
        "last_block_included": utxo.last_block_included
    }
...
def json_destruct_utxo(json_utxo: dict):
    ...
    utxo.last_block_included = json_utxo["last_block_included"]
...
def json_utxo_is_valid(json_utxo: dict):
    try:
        if len(json_utxo) == 3:
            keys = json_utxo.keys()
            if ("utxo_outputs" in keys) and ("balance" in keys) and ("last_block_included" in keys):
                json_utxo_outputs = json_utxo["utxo_outputs"]
                for json_utxo_output in json_utxo_outputs:
